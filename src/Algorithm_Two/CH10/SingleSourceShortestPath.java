...
public class SingleSourceShortestPath {
    ...
    // 최단 경로 트리에 아직 포함되지 않는 정점들의 집합에서 출발 정점으로부터 최소 거리
    ...
    static void printShortestPath(int d[], int p[], int n){
        ...
        // 각 정점의 출발 정점으로부터 최단 경로를 최단 경로트리에서 그 정점의 부모 노드를 거꾸로 따라가면서 출력한다.
        for(int i=0;i<n;i++){
            v = i;
            sp = v + "";
            while(p[v]!=-1){
    ...
    // 출발 정점의 최단 경로 찾는 다익스트라 알고리즘
    // 주어진 그래프는 인접 행렬로 나타낸다.
    static void dijkstra(int W[][], int src, int n){
        int d[] = new int[n]; // d[i]는 출발 정점으롭퉈 정점 i까지 최단 거리를 저장
        // T[i]는 정점 i가 최단 경로트리에 포함된다면 true이다.
        ...
        // 모든 정점의 최다 ㄴ경로 트리에서 부모 노드를 -1로 초기화한다.
        for(int i=0;i<n;i++){
        ...
        for(int count = 0; count < n-1; count++) {
            // 최단 경로 트리에 아직 포함되지 않는 정점들의 집합에서 출발 정점으로부터..
            ...
            for (int v = 0; v < n; v++) {
                // 정점v가 최단 경로 트리에 포함되지 않고 정점 u와 정점 v사이에 간선이 ..
                // 출발 정점부터 u를 거쳐서 v까지 가는 경로의 거리가 d[v]의 현재깝 보다..
                // d[v]를 갱신한다.
    ...
    public static void main(String[] args) {
        ...
        int W[][] = new int[][] {{0,2,INF, INF,7},
                ...
                {INF, INF, 4, 0, 6},
                {7,1,3,6,0}};
        // 출발 정점으로부터 모든 다른 정점까지 가는 최단 경로를 찾아 출력
        dijkstra(W,0,5);
    }
}
